...
void GridSlamProcessor::setMatchingParameters(double urange, double range, double sigma,
                                              int kernsize, double lopt, double aopt, int iterations, double likelihoodSigma,
...
{
    const OdometrySensor *os = dynamic_cast<const OdometrySensor *>(odometry.getSensor());
    if (os && os->isIdeal() && m_outputStream)
    {
        m_outputStream << setiosflags(ios::fixed) << setprecision(3);
        m_outputStream << "SIMULATOR_POS " << odometry.getPose().x << " " << odometry.getPose().y << " ";
        m_outputStream << setiosflags(ios::fixed) << setprecision(6) << odometry.getPose().theta << " " << odometry.getTime() << endl;
    }
}
